...
{
	if (length <= 2 || ticks < 0) { return std::string(length, ' '); }
	...
	for (int i = 0; i < length - 2; i++)
	{
		if (i >= wordStart && i < wordEnd)
		{
			...
			if (i < filledLength) { content += cap(c); }
			else { content += c; }
		}
		else
		{
			if (i < filledLength) { content += '='; }
			else { content += '-'; }
		}
	}
...
std::string ProgressBar::getDisplay()
{
	return display;
}


bool ProgressBar::active()
{
	return ticks >= 0;
}
